...
export function WatchClient({ videoId }: WatchClientProps) {
  ...
  const fetchRoomAndJoin = async (code: string, nextParticipantId?: string | null) => {
    try {
      ...
      const res = await fetch(`/api/rooms/${code}${participantQuery}`);
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    const src = video.hlsPath && video.hlsStatus === "completed" ? video.hlsPath : video.videoUrl;
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      const url = `/api/rooms/${room.code}/messages${query ? `?${query}` : ""}`;
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/rooms/${room.code}/messages`, {
        method: "POST",
  ...
  const syncRoomState = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/rooms/${room.code}${query}`);
      if (!res.ok) {
  ...
  const updateRoomState = async (isPlaying: boolean, currentTime: number) => {
    if (!room) return;
  ...
  const createRoom = async (name: string) => {
    ...
    try {
      ...
      const detailsRes = await fetch(`/api/rooms/${roomData.code}?participantId=${encodeURIComponent(roomData.participantId)}`);
      if (detailsRes.ok) {
        const fullRoom: Room = await detailsRes.json();
        setRoom(fullRoom);
        lastSyncTimeRef.current = new Date(fullRoom.lastUpdatedAt).getTime();
        router.replace(`/watch/${videoId}?room=${roomData.code}`);
      }
    } catch (err: any) {
  ...
  const joinRoom = async (code: string, name: string, silent = false) => {
    ...
    try {
      ...
      const detailsRes = await fetch(`/api/rooms/${code.toUpperCase()}?participantId=${encodeURIComponent(joinData.participantId)}`);
      if (detailsRes.ok) {
  ...
  const handlePlayPause = () => {
    ...
    if (room) {
      updateRoomState(!isPlaying, videoEl.currentTime);
    }
  };
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto px-4 py-20">
          <div className="bg-destructive/10 border border-destructive/50 rounded-lg p-6 flex items-center space-x-3">
            ...
            <p className="text-destructive">{error || "Video no encontrado"}</p>
          </div>
        </div>
      </div>
    );
